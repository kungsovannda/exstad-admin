...
interface ThumbnailForm extends UseFormReturn<MasterProgramFormValues> {
  _thumbnailFile?: File;
}
...
export function ThumbnailUploadField({
  ...
  useEffect(() => {
    const existing = form.getValues("thumbnailUrl");
    if (existing) {
  ...
  const handleFileChange = (newFiles: File[] | null) => {
    ...
    if (newFiles && newFiles.length > 0) {
      ...
      form.setValue("thumbnailUrl", blobUrl, { shouldValidate: false });
    } else {
      setPreviewUrl(null);
      setFiles(null);

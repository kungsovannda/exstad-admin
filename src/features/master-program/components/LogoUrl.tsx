...
import { MasterProgramFormValues } from "@/app/(program)/master-program/create/FormField";

// Extend form to include optional poster File reference
interface PosterForm extends UseFormReturn<MasterProgramFormValues> {
  _logoFile?: File;
}
...
export function LogoUploadField({
  ...
  useEffect(() => {
    const existing = form.getValues("logoUrl");
    if (existing) {
  ...
  const handleFileChange = (newFiles: File[] | null) => {
    ...
    if (newFiles && newFiles.length > 0) {
      ...
      form.setValue("logoUrl", blobUrl, { shouldValidate: false });
    } else {
      setPreviewUrl(null);
      setFiles(null);

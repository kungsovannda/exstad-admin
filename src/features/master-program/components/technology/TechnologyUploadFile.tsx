...
import { TechnologyFormValues } from "./technologyModa";

// Extend form to include optional technology File reference
export interface TechnologyFormWithFile extends UseFormReturn<TechnologyFormValues> {
  _technologyFile?: File;
}

export function TechnologyUploadField({
  form,
  masterProgram,
}: {
  form: TechnologyFormWithFile;
  masterProgram: Partial<MasterProgramType>;
}) {
  const [files, setFiles] = useState<File[] | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  // Preload existing technology when editing
  useEffect(() => {
    const existing = form.getValues("image");
    if (existing) {

...
export default function FaqAdmin({ programUuid }: Props) {
  ...
  const [editingSection, setEditingSection] = useState<{ faqIndex: number; index: number } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<{ type: "topic" | "section"; faqIndex?: number; index?: number } | null>(null);
...
// ✅ Local add/edit FAQ topic
const handleSaveTopicLocal = (data: { title: string }, targetIndex?: number) => {
  let message = "";

  const newFaqs = targetIndex !== undefined
    ? localFaqs.map((r, i) => {
        if (i === targetIndex) {
          message = `FAQ Topic "${data.title}" updated!`;
          return { ...r, title: data.title, faqs: Array.isArray(r.faqs) ? r.faqs : [] };
        }
        return r;
      })
    : (() => {
  ...
  const handleSaveSectionLocal = (faqIndex: number, data: { question: string; answer: string }, sectionIndex?: number) => {
    const newFaqs = [...localFaqs];
    const faq = newFaqs[faqIndex];
    if (!faq) return;

    const safeSections = Array.isArray(faq.faqs) ? [...faq.faqs] : [];
    const updatedSections = sectionIndex !== undefined
      ? safeSections.map((s, i) => i === sectionIndex ? { ...s, question: data.question, answer: data.answer } : s)
      : [...safeSections, { id: crypto.randomUUID(), question: data.question, answer: data.answer }];
...
// ✅ Local delete topic/section with message
const handleDeleteLocal = (
  ...
  if (type === "topic" && faqIndex !== undefined) {
    ...
    // Extract the section title safely
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex justify-between items-center">
        ...
        <AddTopicFaq
          ...
          onSubmit={(data) => { handleSaveTopicLocal(data); setIsCreateOpen(false); }}
          trigger={
      ...
      {(!localFaqs || localFaqs.length === 0) && <div className="text-muted-foreground">No FAQ yet. Add one!</div>}
          ...
          <div key={faq.title || faqIndex} className="flex flex-col gap-2.5 bg-accent rounded-sm p-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2 items-center">
                ...
                <AddTopicFaq
                  ...
                  onSubmit={(data) => { handleSaveTopicLocal(data, faqIndex); setEditingTopicIndex(null); }}
                />
                <FaChevronDown
            ...
            {isExpanded && (
              <div className="flex flex-col gap-2.5 mt-2">
                {sections.map((s, index) => (
                  <div key={index} className="flex flex-col gap-1 p-2.5 bg-background rounded-[4px]">
                    ...
                    <div className="flex gap-2 mt-1 items-center ml-7">
                      <Trash size={16} className="text-destructive cursor-pointer" onClick={() => setDeleteTarget({ type: "section", faqIndex, index })} />
                      <SquarePen size={16} className="text-primary-hover cursor-pointer" onClick={() => setEditingSection({ faqIndex, index })} />
                      <AddQuestionDialog
                        programUuid={programUuid}
                        faqIndex={faqIndex}
                        open={editingSection?.faqIndex === faqIndex && editingSection?.index === index}
                        onOpenChange={(open) => !open && setEditingSection(null)}
                        onSubmit={(data) => handleSaveSectionLocal(faqIndex, data, index)}
                ...
                <Button className="flex items-center w-fit gap-2.5 mt-2" onClick={() => setAddingSectionReqIndex(faqIndex)}>
      ...
      {deleteTarget && (
        <DeleteModal
          ...
          onConfirm={() => deleteTarget && handleDeleteLocal(deleteTarget.type, deleteTarget.faqIndex, deleteTarget.index)}
        />
      )}

      {/* ✅ Save All Button */}
      <div className="flex justify-end mt-4">
        <Button variant={hasChanges ? "default" : "outline"} disabled={!hasChanges} onClick={handleSaveAll}>
          Save All FAQ
        </Button>
      </div>
    </div>

...
const exampleNode : Node<WorkNodeData>[] =  [{
  ...
  const addNewNode = () => {
    ...
    setEditTasks("");
    setEditColor("");
    setEditHandles({ top: "target", right: "target", bottom: "target", left: "target" });
    setIsEditModalOpen(true);
  };
  ...
  const saveEditedNode = () => {
    ...
    if (isAddingNode) {
      ...
      setNodes((nds) =>
        nds.map((node) =>
          node.id === editingNode
            ? { ...node, data: { ...node.data, title: editTitle, tasks: tasksArray, color: editColor, handles: editHandles } }
            : node
        )
      );
    }
  ...
  useEffect(() => {
    ...
    const loadedNodes: Node<WorkNodeData>[] = roadmapData.nodes.map((node, index) => {
      ...
      return {
        ...
        data: { title, tasks: node.data.description?.split(", ").filter((t) => t.trim() !== "") || [], handles, color, onEdit: handleEdit, onDelete: handleDelete },
      };
    });
  ...
  }, [targetRoadmap, masterRoadmap, programType, programUuid, copied, updateRoadmaps, handleEdit, handleDelete, setEdges, setNodes]);
  ...
  const saveAsJson = () => {
    const dataToSave: RoadmapPayload = [
      {
        nodes: nodes.map((node) => {
          const handlesString = positionOrder.map((pos) => node.data.handles[pos]).join(", ");
          return {
            type: "course",
            data: { label: `${node.data.title}, ${handlesString}, ${node.data.color || ""}`, description: node.data.tasks.join(", ") },
            position: node.position,
          };
        }),
    ...
    toast.promise(
      updateRoadmaps({ programUuid, programType, roadmaps: dataToSave }).unwrap(),
      {
        loading: "Saving...",
    ...
    <div className="h-screen w-full flex flex-col">
      ...
      <div className="bg-background border-b p-4 flex items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button onClick={addNewNode}><Plus className="h-4 w-4" />Add Node</Button>
          <Button onClick={saveAsJson} variant="outline">Save</Button>
        </div>
      </div>

      {/* ReactFlow Canvas */}
      <div className="flex-1">
        <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onConnect={onConnect} nodeTypes={nodeTypes} defaultEdgeOptions={{ type: "smoothstep", animated: true }} fitView onEdgeClick={onEdgeClick}>
          <Background />
        </ReactFlow>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="border rounded-lg p-6 bg-muted/30 flex items-center justify-center min-h-[300px]">
            <div className="relative">
              <Card className="min-w-[280px] max-w-[320px] shadow-lg border-2" style={{ backgroundColor: editColor }}>
                {(["top", "right", "bottom", "left"] as const).map((position) => {
                  ...
                  return (
                    <div key={position} className="absolute" style={positions[position]}>
                      {handleType === "source" && <div className="w-5 h-5 rounded-full bg-green-500 border-2 border-white" />}
                      {handleType === "target" && <div className="w-5 h-5 rounded-full bg-blue-500 border-2 border-white" />}
                    </div>
                  );
                })}
                <div className="p-4">
                  <div className="flex items-start justify-between gap-3 mb-3 pb-3 border-b">
                    ...
                    <div className="flex gap-1 shrink-0">
                      <div className="h-8 w-8 rounded-md bg-muted flex items-center justify-center"><Pencil className="h-4 w-4 text-muted-foreground" /></div>
                      <div className="h-8 w-8 rounded-md bg-muted flex items-center justify-center"><Trash2 className="h-4 w-4 text-muted-foreground" /></div>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    {editTasks.trim() === "" && <p className="text-sm text-muted-foreground italic">No tasks yet</p>}
                  </div>
                </div>
              </Card>
            </div>

...
export default function LearningOutcomesAdmin() {
  ...
  const handleEditOutcome = (id: string, data: { title: string; subtitle?: string }) => {
    setOutcomes(prev =>
  ...
  const handleAddSection = (outcomeId: string, data: { title: string }) => {
    setOutcomes(prev =>
      prev.map(o =>
        o.id === outcomeId
          ? { ...o, sections: [...o.sections, { id: Date.now().toString(), title: data.title }] }
          : o
      )
    );
  };

  const handleEditSection = (outcomeId: string, sectionId: string, data: { title: string }) => {
    setOutcomes(prev =>
      prev.map(o =>
        o.id === outcomeId
          ? {
          ...
          <div key={outcome.id} className="flex flex-col gap-2.5 bg-accent rounded-sm p-4">
            <div className="flex justify-between items-center">
                    ...
                    <div   className="flex flex-col cursor-pointer"  >
                      ...
                      {outcome.subtitle && (
                        <span className="text-[12px] text-muted-foreground"> {outcome.subtitle} </span>
                      )}
                    </div>
                    </div>
              ...
              <div className="flex gap-2 items-center">
                <FaTrash size={18}   className="text-destructive cursor-pointer"   onClick={() =>setDeleteTarget({ type: "topic", id: outcome.id }) }  />
                {/* Edit Outcome */}
                <AddTopicDialog
                  initialData={{ title: outcome.title, subtitle: outcome.subtitle }}
                  ...
                  <div key={section.id} className="flex justify-between items-center p-2.5 bg-background rounded-[4px]">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <AddSectionDialog
                        ...
                        onSubmit={(data) => handleEditSection(outcome.id, section.id, data)}
                        trigger={<PiNotePencilFill size={20} className="text-primary-hover cursor-pointer" />}
                      />
                    </div>
                  </div>

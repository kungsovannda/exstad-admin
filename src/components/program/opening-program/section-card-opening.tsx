...
// --- Section with opening program stats ---
export function SectionCardsOpening() {
  ...
  const totalOpening = openingPrograms.length;
  const closedOpening = openingPrograms.filter((p) => p.status === "CLOSED").length;

  // Map programName -> programType for lookup
  const programTypeMap: Record<string, string> = {};
  masterPrograms.forEach((m) => {
    programTypeMap[m.title] = m.programType; // match by title
  });

  let shortCourses = 0;
  let scholarshipCourses = 0;

  openingPrograms.forEach((op) => {
    const type = programTypeMap[op.programName];
    if (type === "SHORT_COURSE") shortCourses++;
    if (type === "SCHOLARSHIP") scholarshipCourses++;
  });

  const stats = [
    { description: "Total Opening Program", value: totalOpening, color: "var(--chart-1)" },
    { description: "Closed Opening Program", value: closedOpening, color: "var(--chart-2)" },
